...
if (-not (Test-CodexDebugPort $Port)) {
  ...
  $launchedForDebug = $true
}

$deadline = (Get-Date).AddSeconds(30)
$setupCheckAt = (Get-Date).AddSeconds(8)
$setupChecked = $false
while (-not (Test-CodexDebugPort $Port)) {
  if ((Get-Date) -ge $deadline) { throw "Codex did not expose CDP on port $Port within 30 seconds." }
  if ($launchedForDebug -and -not $setupChecked -and (Get-Date) -ge $setupCheckAt) {
    $setupChecked = $true
    $debugMain = @(Get-CimInstance Win32_Process -Filter "Name = 'ChatGPT.exe'" -ErrorAction SilentlyContinue |
      Where-Object { $_.CommandLine -match "--remote-debugging-port=$Port(?:\s|$)" })
    $visibleMain = @(Get-Process ChatGPT -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 })
    if ($visibleMain.Count -gt 0 -and $debugMain.Count -eq 0) {
      throw 'WINDOWS_SETUP_REQUIRED: Complete the one-time Windows setup in Codex, approve the UAC prompt, then return to Studio and apply the theme again.'
    }
  }
...
for ($attempt = 0; $attempt -lt 12; $attempt++) {
  ...
  & $node $Injector --verify --port $Port --timeout-ms 1500 *> $null
